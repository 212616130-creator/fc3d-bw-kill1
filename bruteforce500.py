# -*- coding: utf-8 -*-
"""
福彩3D 百位杀一码 — 暴力穷举（最新500期，只算百位）+ TopK 专家池
=====================================================================
公式池：81特征 × 单/双/三特征线性组合 ≈ 2356万规格（与 formulas.iter_specs 同规则）。
numpy 向量化计算 500 期输出；每族独立小顶堆(pfl=34)在线维护专家池，合并后取 TopK=240。
...
TOPK = 240            # 专家池规模上限
PFL = 34              # 每族限选上限（7族 × 34 ≈ 238 ≈ TOPK；控制多样性）
POOL_TOTAL = 3240 + 518400 + 23036400   # 单+双+三特征公式总数 = 23,558,040（81特征）
...
def search_pool(hh, tt, oo, window=WINDOW, topk=TOPK, pfl=PFL, verbose=True):
    """905万公式 × 最新window期穷举（只算百位），按族限选在线维护专家池。
    ...
    start = N - window
    if verbose:
        print(f"穷举窗口: 数据第 {start+1}..{N} 期，共 {window} 期（只算百位）")

    # 特征矩阵 (window, 59)，int16 省内存带宽；第k行由 期start+k-1 + 前2期 计算
    rows = [
        feat_list(
    ...
    F = np.array(rows, dtype=np.int16)                 # (500,59)
    ah = np.array(hh[start:start + window], dtype=np.int16)   # 被预测的500个百位
    ...
    for idx in range(NF):
        ...
        for c in COEFFS:
            ...
            for const in range(10):
                out = (base + const) % 10
                hits = int((out != ah).sum())
                _update_heap(fam_heaps, 'A' if idx < 34 else 'B' if idx < 43 else 'C',
                             hits, ((c, idx),), const, seq, pfl)
                seq += 1
    if verbose:
        _fmt_progress(2360, POOL_TOTAL, t0, "单特征")

    # ---- 双特征: C(59,2)×16×10 = 273,760 ----
    done = 2360
    for i in range(NF):
        Fi = F[:, i]
